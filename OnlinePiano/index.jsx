...
const keyDownFunc = (ev) => {
  ...
  if (key1 !== "error") {
    ...
    document
      ...
      .classList.add("highlightedControlButton");
  } else if (ev.key == "x") {
    increaseOctave();
    document
  ...
  if (key2 !== "error") {
    ...
    switch(ev.key.toLowerCase()) {
      ...
      case "x":
        ...
        break;
      case ",":
        document.getElementById("previousInstrumentButton").classList.remove("highlightedControlButton")
        break;
      case ".":
        document.getElementById("nextInstrumentButton").classList.remove("highlightedControlButton")
        break;
    }
  }
}
  ...
  return (
    <div id="controlsDiv">
      <div id="octaveControls">
        <a
          id="decreaseOctaveButton"
          className="controlButtons"
          onClick={decreaseOctave}
        >
          <p>{`<`}</p>
        </a>
        <p id="octaveText">{`C${octave}`}</p>
        <a
          id="increaseOctaveButton"
          className="controlButtons"
      ...
      <div id="instrumentControls">
        <a
          ...
          id="previousInstrumentButton"
          className="controlButtons"
        ><p>{`<`}</p></a>
        <p id="instrumentName">{instruments[instrumentIndex].name}</p>
        <a
          onClick={nextInstrument}
          id="nextInstrumentButton"
          className="controlButtons"
        ><p>{`>`}</p></a>
      </div>
    </div>
...
function WhiteKey(props) {
  return (
    <div
      id={props.id}
      class="whiteKey"
      onMouseDown={is_iOS ? null: () => playKey(props.id)}
      onMouseUp={is_iOS ? null :() => stopKey(props.id)}
      onMouseOut={is_iOS ? null : () => stopKey(props.id)}
      onTouchStart={() => playKey(props.id)}
      onTouchEnd={() => stopKey(props.id)}
    >
      <p class="keyLabel">{`${props.id[0]}`}</p>
    </div>
  );
}
...
function playKey(key1) {
  highlightKey(key1);
  const noteToPlay = key1[key1.length - 1] != "1"
    ? key1 + `${octave}`
      ...
      if (!is_iOS) {
        ...
        sprite.on(
          ...
          () => {
            ...
            allSoundsPlaying.splice(
              allSoundsPlaying.indexOf(associatedObj.soundId),
              1
            );
          },
...
function refreshInstrument() {
  ...
  sprite.load();
  if(document.getElementById("instrumentName")) document.getElementById("instrumentName").innerHTML = `${instruments[instrumentIndex].name}`;
}
